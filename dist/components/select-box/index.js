...
import { pressingModifier, startsWithString } from '../../-private/utils.js';
import { task } from 'ember-concurrency';
import { tracked } from 'tracked-built-ins';
import Component from '@glimmer/component';
import lifecycle from '../../modifiers/lifecycle.js';
import Dropdown from '../dropdown/index.js';
import SelectBoxGroup from './group.js';
import SelectBoxInput from './input.js';
import SelectBoxOption from './option.js';
import SelectBoxOptions from './options.js';
...
class SelectBox extends Component {
  static {
    g(this.prototype, "_activeOption", [tracked]);
  }
  #_activeOption = (i(this, "_activeOption"), void 0);
  static {
    g(this.prototype, "_options", [tracked], function () {
      return tracked(Set);
    });
  }
  #_options = (i(this, "_options"), void 0);
  static {
    g(this.prototype, "dropdown", [tracked]);
  }
  #dropdown = (i(this, "dropdown"), void 0);
  static {
  ...
  static {
    g(this.prototype, "optionsElement", [tracked]);
  }
  #optionsElement = (i(this, "optionsElement"), void 0);
  static {
  ...
  static {
    g(this.prototype, "keyboard", [tracked]);
  }
  ...
  static {
    g(this.prototype, "results", [localCopy('args.options')]);
  }
  ...
  static {
    n(this.prototype, "activeOption", [cached]);
  }
  get specificActiveOption() {
    return this.options.includes(this._activeOption) ? this._activeOption : null;
  }
  get optionForValue() {
  ...
  static {
    n(this.prototype, "options", [cached]);
  }
  get orderedOptions() {
  ...
  static {
    n(this.prototype, "orderedOptions", [cached]);
  }
  ...
  #getOptionsByTextContent(options, query) {
    return filter(options).by(option => option.element.textContent).query(query).using(startsWithString).run();
  }
  #buildSelection(value) {
    const build = this.args.onBuildSelection ?? this.#defaultBuildSelection.bind(this);
  ...
  static {
    setComponentTemplate(precompileTemplate("{{!-- template-lint-disable no-invalid-interactive no-pointer-down-event-binding --}}\n{{~this.registerComponents (hash Input=(component SelectBoxInput disabled=this.isDisabled aria-busy=this.isBusy aria-activedescendant=this.activeOption.element.id aria-expanded=this.dropdown.isOpen aria-controls=this.optionsElement.id onInsert=this.handleInsertInput onDestroy=this.handleDestroyInput onInput=this.handleInput onKeyDown=this.handleKeyDownInput) Group=SelectBoxGroup Options=(component SelectBoxOptions aria-multiselectable=this.isMultiple aria-activedescendant=this.optionsActiveDescendant tabindex=this.optionsTabIndex onInsert=this.handleInsertOptions onDestroy=this.handleDestroyOptions onKeyDown=this.handleKeyDownOptions) Option=(component SelectBoxOption selectBox=this onInsert=this.handleInsertOption onDestroy=this.handleDestroyOption onMouseEnter=this.handleMouseEnterOption onMouseUp=this.handleMouseUpOption onKeyDown=this.handleKeyDownOption onFocusIn=this.handleFocusInOption) Dropdown=(component Dropdown disabled=this.isDisabled onReady=this.registerDropdown onOpenClosure=this.handleOpenDropdown onCloseClosure=this.handleCloseDropdown onDestroy=this.handleDestroyDropdown) Trigger=(component this.dropdown.Trigger role=this.triggerRole aria-busy=this.isBusy aria-disabled=this.isDisabled aria-activedescendant=this.triggerActiveDescendant aria-controls=this.optionsElement.id aria-haspopup=null tabindex=this.triggerTabIndex onInsertClosure=this.handleInsertTrigger onDestroy=this.handleDestroyTrigger onKeyDown=this.handleKeyDownTrigger) Content=(component this.dropdown.Content tabindex=\"-1\"))~}}\n<div class=\"select-box\" data-busy=\"{{this.isBusy}}\" data-disabled=\"{{this.isDisabled}}\" {{on \"mouseleave\" this.handleMouseLeave}} {{on \"mousedown\" this.handleMouseDown}} {{lifecycle onInsert=this.handleInsertElement onDestroy=this.handleDestroyElement}} ...attributes>\n  {{~yield this.api~}}\n</div>", {
      strictMode: true,
      scope: () => ({
        hash,
        SelectBoxInput,
