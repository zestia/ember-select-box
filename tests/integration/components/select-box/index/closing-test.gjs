...
module('select-box (closing)', function (hooks) {
  ...
  test('closing with api', async function (assert) {
    ...
    await render(<template>
      <SelectBox as |sb|>
        <sb.Dropdown @open={{true}} @onClose={{handleClose}} as |dd|>
          <button type="button" class="test" {{on "click" dd.close}}></button>
          <sb.Trigger />
          <sb.Input />
        </sb.Dropdown>
      </SelectBox>
  ...
  test('closing due to touching outside', async function (assert) {
    ...
    // iOS devices retain focus when tapping outside,
    // so our usual focus-leave technique for closing won't work.
    // This is contrary to pointer devices.
