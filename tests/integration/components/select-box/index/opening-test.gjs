...
module('select-box (opening)', function (hooks) {
    ...
    await render(<template>
      <SelectBox as |sb|>
        <sb.Dropdown @onOpen={{handleOpen}} as |dd|>
          <button type="button" class="test" {{on "click" dd.open}}></button>
          <sb.Trigger />
          <sb.Input />
        </sb.Dropdown>
      </SelectBox>
  ...
  todo('dropdown trigger not available', async function (assert) {
    ...
    assert.dom('.dropdown__trigger').doesNotExist(`
      ...
      trigger should be used, not a plain trigger.
      Sadly, not that easy to prevent accidental use of this.
    `);
  });
});
