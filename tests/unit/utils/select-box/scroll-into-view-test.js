...
import scrollIntoView from '@zestia/ember-select-box/utils/dom/scroll-into-view';
...
module('select-box (scroll into view util)', function(hooks) {
  ...
  test('scroll up', function(assert) {
    ...
    assert.equal(container.scrollTop, 24, 'scrolls to the element');
  });

  test('scroll down', function(assert) {
    assert.expect(2);

    assert.equal(container.scrollTop, 0, 'precondition, not scrolled');

    scrollIntoView(bottom);

    assert.equal(container.scrollTop, 21, 'scrolls to the element');
  });
});
